...
void main() {
  ...
  test('Special case 1', () {
    ...
    final s1 = sss.combine(shards.sublist(0, 2), true);
    print('secret: $s1');

    final s2 = sss.combine(shards.sublist(1), true);
    print('secret: $s2');

    final s3 = sss.combine(shards, true);
    print('secret: $s3');
  });
  ...
  test('Special case 4', () {
    const secret = 'hello1234';
    final encodedSecret = base64UrlEncode(utf8.encode(secret));
    final shards = sss.create(2, 3, encodedSecret, true);

    print('secret: $secret');
    print('secretEncoded: $encodedSecret');
    print(shards);

    final s1 = sss.combine(shards.sublist(0, 2), true);
    print('secret: ${utf8.decode(base64Decode(s1))}');

    final s2 = sss.combine(shards.sublist(1), true);
    print('secret: ${utf8.decode(base64Decode(s2))}');

    final s3 = sss.combine(shards, true);
    print('secret: ${utf8.decode(base64Decode(s3))}');
  });
}
