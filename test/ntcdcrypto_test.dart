...
import '../lib/src/ntcdcrypto.dart';

void main() {
  final testSecret = 'nghiatc'.padRight(109, 'x');

  group('Utils', () {
    test('trimRightDoubledZero', () {
      expect(trimRightDoubledZero(''), '');
      expect(trimRightDoubledZero('1'), '1');
      expect(trimRightDoubledZero('12'), '12');
      expect(trimRightDoubledZero('123'), '123');
      expect(trimRightDoubledZero('12345'), '12345');
      expect(trimRightDoubledZero('123456'), '123456');
      expect(trimRightDoubledZero('123450'), '123450');
      expect(trimRightDoubledZero('1234560'), '1234560');
      expect(trimRightDoubledZero('123000'), '1230');
      expect(trimRightDoubledZero('1230000'), '123');
    });

    test('Test split & merge', () {
      final chunks = splitSecretToChunks(testSecret);
      expect(
          chunks.contains(BigInt.parse(
            '49937119214509114343548691117920141602615245118674498473442528546336026425464',
            radix: 10,
          )),
          true);
      expect(combineChunksToSecret(chunks), testSecret);
    });
  });

  group('Test .split and .combine', () {
    test('Test combine Hex', () {
      ...
      expect(
        restoreSecret(shares: shares.sublist(0, 3), isBase64: false),
        testSecret,
      );
      expect(
        restoreSecret(shares: shares.sublist(3, 6), isBase64: false),
        testSecret,
      );
      expect(
        restoreSecret(shares: shares.sublist(1, 5), isBase64: false),
        testSecret,
      );
    });

    test('Test split & combine Hex', () {
    ...
    test('Test combine Base64Url', () {
      ...
      expect(restoreSecret(shares: shares.sublist(0, 3)), testSecret);
      expect(restoreSecret(shares: shares.sublist(3, 6)), testSecret);
      expect(restoreSecret(shares: shares.sublist(1, 5)), testSecret);
    });

    test('Test split & combine Base64Url', () {
      final shares = splitSecret(
        secret: testSecret,
        treshold: 3,
        shares: 6,
  ...
  group('Special cases for test right padding and trimming with 0x00', () {
    test('Special case 1', () {
      const secret = 'бар';
      final shares = splitSecret(
        secret: secret,
      ...
      expect(restoreSecret(shares: shares), secret);
      expect(restoreSecret(shares: shares.sublist(0, 2)), secret);
      expect(restoreSecret(shares: shares.sublist(1, 3)), secret);
    });
    ...
    test('Special case 4', () {
      final encodedSecret = base64UrlEncode(utf8.encode('hello1234'));
      final shares = splitSecret(
        secret: encodedSecret,
        treshold: 2,
        shares: 3,
      );
      expect(restoreSecret(shares: shares), encodedSecret);
      expect(restoreSecret(shares: shares.sublist(0, 2)), encodedSecret);
      expect(restoreSecret(shares: shares.sublist(1, 3)), encodedSecret);
    });
  });
}
